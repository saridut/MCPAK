module m_spline
    !! Implements various routines for cubic spline interpolation.
...
!  Input:
!
!    real ( kind = 8 ) TDATA(5), the nodes associated with the
!    basis function.  The entries of TDATA are assumed to be distinct
!    and increasing.
!
!    real ( kind = 8 ) TVAL, a point at which the B spline basis
!    function is to be evaluated.
!
!  Output:
!
!    real ( kind = 8 ) YVAL, the value of the function at TVAL.
!
  implicit none
...
!    to:
!      a2(i) = ( t(i)   - t(i-1) ) / 6.0D+00
!      a3(i) = ( t(i+1) - t(i-1) ) / 3.0D+00
!      a4(i) = ( t(i+1) - t(i)   ) / 6.0D+00
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!  Input:
!
!    integer ( kind = 4 ) N, the number of data points; N must be
!    at least 2.
!
!    real ( kind = 8 ) T(N), the points where data is specified.
!    The values should be distinct, and increasing.
!
!    real ( kind = 8 ) Y(N), the data values to be interpolated.
!
!    integer ( kind = 4 ) IBCBEG, the left boundary condition flag:
!    0: the spline should be a quadratic over the first interval;
!    1: the first derivative at the left endpoint should be YBCBEG;
!    2: the second derivative at the left endpoint should be YBCBEG;
!    3: Not-a-knot: the third derivative is continuous at T(2).
!
!    real ( kind = 8 ) YBCBEG, the left boundary value, if needed.
!
!    integer ( kind = 4 ) IBCEND, the right boundary condition flag:
!    0: the spline should be a quadratic over the last interval;
!    1: the first derivative at the right endpoint should be YBCEND;
!    2: the second derivative at the right endpoint should be YBCEND;
!    3: Not-a-knot: the third derivative is continuous at T(N-1).
!
!    real ( kind = 8 ) YBCEND, the right boundary value, if needed.
!
!  Output:
!
!    real ( kind = 8 ) YPP(N), the second derivatives of
!    the cubic spline.
!
  implicit none
...
!
!    real ( kind = 8 ) T(N), the knot values.
!
!    real ( kind = 8 ) Y(N), the data values at the knots.
!
!    real ( kind = 8 ) YPP(N), the second derivatives of the
!    spline at the knots.
!
!    real ( kind = 8 ) TVAL, a point, typically between T(1) and
!    T(N), at which the spline is to be evalulated.  If TVAL lies outside
!    this range, extrapolation is used.
!
!  Output:
!
!    real ( kind = 8 ) YVAL, YPVAL, YPPVAL, the value of the spline, and
